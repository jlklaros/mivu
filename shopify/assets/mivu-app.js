...
function preloadFrames(onProgress) {
  return new Promise(resolve => {
    ...
    for (let i = 1; i <= FRAME_COUNT; i++) {
      ...
      img.src = `https://jlklaros.github.io/mivu/frames/frame_${String(idx).padStart(4, '0')}.webp`;
      frames[idx] = img;
    }
  });
}
...
───────────────────────────────────────────────────── */
/* ─────────────────────────────────────────────────────
   Native Shopify cart — pack selector + form submission
───────────────────────────────────────────────────── */
function setupShopify() {
  let selectedVariantId = null;

  /* ── Pack selector: update hidden form input + UI ── */
  document.querySelectorAll('.pack-option').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      /* Update hidden form input so the form posts the right variant */
      const variantInput = document.getElementById('mivu-variant-id');
  ...
  /* ── Floating CTA + scroll CTA buttons → go to #purchase ── */
  /* (these are <a href="#purchase"> so no extra JS needed)   */

  /* ── Add to Bag form: submit → redirect to checkout ── */
  const form = document.getElementById('mivu-cart-form');
  if (form) {
    form.addEventListener('submit', async e => {
      e.preventDefault();
      const variantId = document.getElementById('mivu-variant-id')?.value;
      if (!variantId) return;
